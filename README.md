...
| 版 | 日付 | 作成者 | 概要 |
| --- | --- | --- | --- |
| 0.1 | 2025‑07‑15 | Hikaru Miyashita | 初版ドラフト作成 |
...
![バイオリン弓の試作例](./IMG_3296.jpeg)

※上記は小林様が試作されたバイオリン弓です。I型ロードセル（携帯用デジタルスケール分解品）を使用し、軽量化を重視した設計となっています。

#### ロードセル配線詳細
写真中の4本配線（黒・赤・緑・白）の役割：
```
赤線（E+） : 励起電圧プラス（3.3V〜5V電源）
黒線（E-） : 励起電圧マイナス（GND）
白線（A+） : 信号出力プラス（荷重検出）
緑線（A-） : 信号出力マイナス（荷重検出）
```

**動作原理**：励起電圧（赤・黒）でロードセル内部のホイートストンブリッジを駆動し、荷重により生じる微小電圧差を信号線（白・緑）から差動出力として取得。
...
**小林様提供：バイオリン運弓センシング概論**  
[運弓センシング概論.pdf](./概論_20250706.pdf)
